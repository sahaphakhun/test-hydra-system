...
import { LineConfig, ILineConfig } from '../models/lineConfig';
...
router.get('/', async (req: Request, res: Response) => {
  try {
    const lineConfigs = await LineConfig.find().select('-channelSecret -channelAccessToken');
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const lineConfig = await LineConfig.findById(req.params.id).select('-channelSecret -channelAccessToken');
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const existingConfig = await LineConfig.findOne({ name });
    if (existingConfig) {
      return res.status(400).json({ success: false, message: 'ชื่อการตั้งค่านี้มีอยู่แล้ว' });
    }
    
    // สร้างการตั้งค่าใหม่
    const lineConfig = await LineConfig.create({
      name,
      channelId,
      channelSecret,
      channelAccessToken
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const lineConfig = await LineConfig.findById(req.params.id);
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    const lineConfig = await LineConfig.findByIdAndDelete(req.params.id);
...
export const lineConfigRoutes = router; 