...
const getAllAccounts = async (req, res) => {
    try {
        const accounts = await LineAccount_1.default.find().select('-password');
...
const getAccountById = async (req, res) => {
    try {
        ...
        const account = await LineAccount_1.default.findById(accountId).select('-password');
        if (!account) {
...
const addFriends = async (req, res) => {
    try {
        ...
        const account = await LineAccount_1.default.findById(accountId);
        if (!account) {
            return res.status(404).json({ message: 'ไม่พบบัญชีที่ต้องการ' });
        }
...
const sendMessageToGroup = async (req, res) => {
    try {
        ...
        const group = await LineGroup_1.default.findById(groupId);
        if (!group) {
